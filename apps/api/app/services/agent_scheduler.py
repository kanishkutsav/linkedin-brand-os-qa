...
class AgentScheduler:
    ...
    async def _loop(self) -> None:
        ...
        while True:
            now = datetime.now(tz)
            if settings.agent_in_process_schedule_enabled and not settings.durable_scheduled_worker_enabled and settings.agent_daily_discovery_enabled:
                if (
            ...
            if settings.agent_in_process_schedule_enabled and not settings.durable_scheduled_worker_enabled and settings.agent_calendar_enabled:
                if (
            ...
            async with self.session_factory() as learning_session:
                ...
                if settings.agent_in_process_schedule_enabled and not settings.durable_scheduled_worker_enabled and self._last_retention_date != now.date().isoformat():
                    try:
